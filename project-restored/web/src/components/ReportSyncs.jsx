...
import { getReportSyncs, getReportStatuses, updateReportSyncStatus } from '../services/api';
...
function ReportSyncs() {
  ...
  const fetchReportSyncs = async () => {
    try {
      const data = await getReportSyncs();
      setReportSyncs(data);
  ...
  const fetchReportStatuses = async () => {
    try {
      const data = await getReportStatuses();
  ...
  const updateStatus = async (id, newStatusId, newProgress = null) => {
    setUpdatingId(id);
    
    // Find current sync to get values if not provided
    const currentSync = reportSyncs.find(s => s.id === id);
    const statusToUpdate = newStatusId || currentSync.report_status_id;
    const progressToUpdate = newProgress !== null ? newProgress : currentSync.progress;

    try {
      const result = await updateReportSyncStatus(id, statusToUpdate, progressToUpdate);
      
      // Mettre à jour localement
      setReportSyncs(prev => 
        prev.map(sync => {
          if (sync.id === id) {
            const newStatus = reportStatuses.find(s => s.id === parseInt(statusToUpdate));
            return {
              ...sync,
              report_status_id: parseInt(statusToUpdate),
              ...
              {filteredReportSyncs.map((sync) => (
                <tr key={sync.id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    ...
                    <div style={styles.progressInputContainer}>
                      <input 
                        type="range" 
                        min="0" 
                        max="100" 
                        ...
                        onChange={(e) => {
                          ...
                          setReportSyncs(prev => prev.map(s => s.id === sync.id ? {...s, progress: val} : s));
                          setFilteredReportSyncs(prev => prev.map(s => s.id === sync.id ? {...s, progress: val} : s));
                        }}
                        onMouseUp={(e) => updateStatus(sync.id, null, parseInt(e.target.value))}
                        onTouchEnd={(e) => updateStatus(sync.id, null, parseInt(e.target.value))}
